...
const all: BuiltinHandler = async (args) => {
  const promises = (Array.isArray(args[0]) ? args[0] : []) as Promise<unknown>[];
...
const retry: BuiltinHandler = async (args) => {
  ...
  for (let i = 0; i <= maxRetries; i++) {
    try { return await fn(); } catch (err) {
      lastError = err;
      if (i < maxRetries) await new Promise((r) => setTimeout(r, backoff ? delayMs * Math.pow(2, i) : delayMs));
    }
  }
  throw lastError;
};
...
const parallel: BuiltinHandler = async (args) => {
  const fns = (Array.isArray(args[0]) ? args[0] : []) as (() => Promise<unknown>)[];
...
const waterfall: BuiltinHandler = async (args) => {
  const fns = (Array.isArray(args[0]) ? args[0] : []) as ((prev: unknown) => Promise<unknown>)[];
...
const map: BuiltinHandler = async (args) => {
  ...
  const fn = args[1] as (item: unknown, index: number) => Promise<unknown>;
...
const filter: BuiltinHandler = async (args) => {
  const items = (Array.isArray(args[0]) ? args[0] : []) as unknown[];
  const fn = args[1] as (item: unknown) => Promise<boolean>;
...
const each: BuiltinHandler = async (args) => {
  const items = (Array.isArray(args[0]) ? args[0] : []) as unknown[];
  const fn = args[1] as (item: unknown, index: number) => Promise<void>;
...
const reduce: BuiltinHandler = async (args) => {
  const items = (Array.isArray(args[0]) ? args[0] : []) as unknown[];
  const fn = args[1] as (acc: unknown, item: unknown) => Promise<unknown>;
  let acc = args[2];
...
const debounce: BuiltinHandler = (args) => {
  ...
  return (...a: unknown[]) => {
    ...
    return new Promise((resolve) => { timer = setTimeout(() => resolve(fn(...a)), ms); });
  };
};
...
export const PromiseFunctionMetadata: Record<string, FunctionMetadata> = {
  ...
  waterfall: { description: "Run functions in sequence, passing results", parameters: [{ name: "fns", dataType: "array", description: "Functions (prev) => result", formInputType: "text", required: true }, { name: "initial", dataType: "any", description: "Initial value", formInputType: "text", required: false }], returnType: "any", returnDescription: "Final result", example: 'promise.waterfall [$step1, $step2]' },
  map: { description: "Map items with async function", parameters: [{ name: "items", dataType: "array", description: "Items", formInputType: "text", required: true }, { name: "fn", dataType: "function", description: "Async mapper", formInputType: "text", required: true }, { name: "concurrency", dataType: "number", description: "Max concurrent", formInputType: "text", required: false }], returnType: "array", returnDescription: "Mapped results", example: 'promise.map $urls $fetchFn 5' },
  ...
  throttle: { description: "Throttle function calls", parameters: [{ name: "fn", dataType: "function", description: "Function", formInputType: "text", required: true }, { name: "ms", dataType: "number", description: "Min interval ms", formInputType: "text", required: true }], returnType: "function", returnDescription: "Throttled function", example: 'promise.throttle $fn 1000' },
  debounce: { description: "Debounce function calls", parameters: [{ name: "fn", dataType: "function", description: "Function", formInputType: "text", required: true }, { name: "ms", dataType: "number", description: "Debounce delay ms", formInputType: "text", required: true }], returnType: "function", returnDescription: "Debounced function", example: 'promise.debounce $fn 300' },
...
export const PromiseModuleMetadata: ModuleMetadata = {
